...
# MapCycle (multi-lignes supporté)
...
# Fix permissions fichiers config
touch "${MAPCYCLE}" "${GAMEINI}"
chmod 664 "${MAPCYCLE}" "${GAMEINI}"

# ────────────────────────────────
# (A) Auto-update via SteamCMD
# ────────────────────────────────
if [ "${AUTO_UPDATE}" = "1" ]; then
  ...
  "${STEAMCMDDIR}/steamcmd.sh" +@sSteamCmdForcePlatformType linux \
    ...
    +login anonymous \
    +app_update "${APPID}" -beta public \
    +quit || echo "⚠️ SteamCMD update failed, continuing anyway"
fi
